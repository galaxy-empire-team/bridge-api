...
func Mist(missionService MissionService) func(c *gin.Context) {
...
func handleMistError(c *gin.Context, err error) {
	switch {
	...
	case errors.Is(err, models.ErrFleetCannotBeEmpty):
		c.JSON(http.StatusForbidden, ErrorResponse{
			Err: "fleet cannot be empty for mist mission",
		})
	default:

...
func (s *Service) StartFleetConstruction(ctx context.Context, userID uuid.UUID, planet uuid.UUID, fleet models.FleetUnitCount) error {
	...
	if err != nil {
		return fmt.Errorf("planetStorage.CheckFleetConstruction(): %w", err)
	}
	if fleetConstructionInProgress {
		return models.ErrFleetConstructionInProgress
	}
...
func (s *Service) generateEventForFleetConstruct(ctx context.Context, planetID uuid.UUID, fleet models.FleetUnitCount, planetRepo TxStorages) (models.FleetConstructEvent, error) {
	...
	leftResources := models.Resources{
		Metal:     resources.Metal - fleetUnitStats.MetalCost,
		Crystal:   resources.Crystal - fleetUnitStats.CrystalCost,
		Gas:       resources.Gas - fleetUnitStats.GasCost,
